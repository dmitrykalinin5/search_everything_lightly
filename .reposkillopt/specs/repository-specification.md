...
Status: implemented MVP 0.1.0; physical desktop acceptance pending.  
Generated by: repo-skillopt 0.10.0 · Codex adapter · 2026-09-09.  
...
**[fact]** `metadata.json:1` — GNOME Shell extension for local filename search. Initially only the supplied technical specification existed; implementation is new, not a legacy migration.
...
**[fact]** `extension.js:12`, `src/searchOverlay.js:16`, `src/searchEngine.js:15`, `src/fileActions.js:46` — lifecycle owns UI and engine; UI schedules search and file actions; engine owns subprocesses and ranking; file actions own Gio/DBus operations. Pure query/ranking modules do not import GNOME UI libraries.

## 6. Core modules

**[fact]** `src/query.js:18` — user terms become escaped literal substring arguments after `--`. `src/ranking.js:31` sorts distinct paths with name quality ahead of location priority. `src/resultItem.js:12` creates a row and queries its icon asynchronously. `src/searchEngine.js:24` terminates and cancels the active request. `src/searchOverlay.js:104` invalidates UI generations, debounce and pending actions. `src/fileActions.js:46` handles open, parent and reveal actions.

### Symbols not yet analyzed

52 defined, 24 analyzed, 28 listed (named functions/classes). Anonymous callbacks belong to their enclosing methods.
...
- `abbreviatePath()` — `src/query.js:30`
- `displayText()` — `src/query.js:36`

**src/ranking.js**

- `normalize()` — `src/ranking.js:4`
- `matchRank()` — `src/ranking.js:8`
- `locationRank()` — `src/ranking.js:21`

**src/searchEngine.js**

- `.constructor()` — `src/searchEngine.js:9`
- `._clearTimer()` — `src/searchEngine.js:35`

**src/searchOverlay.js**

- `.constructor()` — `src/searchOverlay.js:17`
- `.close()` — `src/searchOverlay.js:85`
- `.destroy()` — `src/searchOverlay.js:92`
- `._setStatus()` — `src/searchOverlay.js:128`
- `._select()` — `src/searchOverlay.js:189`
- `._activate()` — `src/searchOverlay.js:231`
...
|---|---|---|---|
| module | 8 production JS files | `extension.js:1`, `prefs.js:1`, `src/query.js:1` | **[fact]** |
| class | 6 production classes | `extension.js:11`, `src/searchEngine.js:8`, `src/resultItem.js:12` | **[fact]** |
| function | Complete counts in symbols.json | `.reposkillopt/specs/symbols.json` | **[fact]** |
| data entity | query, request, path, shortcut | `src/query.js:12`, `src/searchEngine.js:63`, `schemas/org.gnome.shell.extensions.search-everything-lightly.gschema.xml:5` | **[fact]** |
...
No application database or custom file index. An ER diagram is not applicable.
...
| query / result paths | transient UI state | `src/searchOverlay.js:120` | **[fact]** |
...
2. **[fact]** `src/searchOverlay.js:133` clears old work and schedules `_search` after debounce.
3. **[fact]** `src/searchEngine.js:42` collects subprocess output asynchronously and ranks candidates.
4. **[fact]** `src/searchOverlay.js:162` checks the generation before creating rows.
5. **[fact]** `src/searchOverlay.js:206`, `src/fileActions.js:46` dispatch the selected action to Gio.
...
1. **[fact]** `src/query.js:18` escapes metacharacters without involving a shell.
2. **[fact]** `src/searchEngine.js:75` communicates using bytes, then splits NUL-delimited UTF-8 paths.
3. **[fact]** `src/ranking.js:31` deduplicates and limits results; `src/resultItem.js:22` sanitizes control characters only for display.
...
| overlay | ModalDialog, ResultItem, file actions, query limits | `src/searchOverlay.js:2` | **[fact]** |
...
|---|---|---|---|
| minimum / maximum query | 3 / 256 characters | `src/query.js:2` | **[fact]** |
...
**[fact]** `tests/query.test.js:1`, `tests/ranking.test.js:1` — 10 pure logic checks. `tests/integration.js:1` — 13 GJS/plocate cases. `tests/shellSmoke.js:run` — real headless Shell, virtual-keyboard hotkey, Gio launch recorder, FileManager1 success/fallback, focus/scrolling and lifecycle. The test driver is injected only into a disposable copy (`tests/run_shell_smoke.py:main`).

## 15. Deployment assumptions

**[fact]** `metadata.json:1`, `Makefile:13` — UUID `search-everything-lightly@ogultra`; user extension installation; GNOME 49 only. **[unknown]** Compatibility with other Shell versions and distributions is not established. GNOME session restart is required after changing loaded JS on Wayland (`README.md`).
...
**[fact]** `src/query.js:4` — only the first 300 plocate candidates are reranked, so a better match outside that set cannot be shown. `README.md` documents stale/pruned indexes and the default keyboard-layout conflict. **[inference]** Private ModalDialog monitor positioning may require changes for future Shell versions; basis: `src/searchOverlay.js:80` uses `_monitorConstraint` after the installed GNOME 49 implementation was inspected.

## 18. Unknowns and unresolved questions

**[unknown]** Physical multi-monitor positioning, fractional/HiDPI scaling, screen locking, user-specific themes, and clean installation into a live account still require the checks in `tests/MANUAL.md`. Other distributions and GNOME versions are not validated. Public repository URL and final publication ownership/UUID are not supplied. No EGO publication was attempted.
