...
Локальный поиск приложений, файлов и папок в компактном окне GNOME Shell. Версия **0.5.0**, Fedora / GNOME Shell **49 и 50**, Wayland. Английский интерфейс и русский перевод через gettext.
...
- Раскрытая панель полупрозрачна и не отбрасывает тень. Единственный результат сохраняет обычную высоту строки, а оставшаяся часть фиксированной панели остаётся пустой.
...
Она использует отдельный D-Bus, два виртуальных монитора 1280×900, временные настройки и базу. Настоящая виртуальная клавиатура проверяет глобальную горячую клавишу; Enter, Ctrl+Enter и Shift+Enter проверяются с временным обработчиком файлов и тестовым FileManager1. Проверяются выбор монитора активного окна, фокус, прокрутка, размеры строк, повторное включение и открытие GTK-настроек. Скриншоты и журнал сохраняются в указанном скриптом каталоге `/tmp/sel-shell-*`; текущий рабочий сеанс не изменяется. Эта проверка требует окружения, разрешающего D-Bus и запуск headless Mutter.
...
Совместимость проверена на GNOME Shell **49.9** (Fedora 43) и **50.4** (контейнер Fedora 44): на каждой версии прошли 17 headless UI-сценариев и 19 интеграционных тестов GJS. В контейнере используется программный рендеринг; физические мониторы, дробное масштабирование и блокировка сеанса остаются в ручной проверке.

## Дальнейшие версии

- Следующее обновление: дополнительные настройки источников и исключённых каталогов.
