...
Локальный поиск приложений, файлов и папок в компактном окне GNOME Shell. Версия **0.4.0**, Fedora / GNOME Shell **49**, Wayland. Английский интерфейс и русский перевод через gettext.
...
- Появление окна анимировано мягким `fade + scale` за 180 мс. Промежуточная надпись во время поиска не показывается.
...
В настройках доступны смена сочетания клавиш и состояние источников поиска. Там же объясняется, что LocalSearch обновляется автоматически, а `sudo updatedb` нужен только при устаревших необязательных результатах plocate. GSettings-схема: `org.gnome.shell.extensions.search-everything-lightly`, ключ `toggle-search`, тип `as`.
